...
#from tkinter.ttk import Frame  # for Linux?
...
class Window(Frame):
    ...
    def initWindow(self, figure):
        ...
        self.figure_frame = Frame(self.master, relief=Tk.RAISED)

        #canvas = FigureCanvasTk(figure, master=self.master)
        canvas = FigureCanvasTk(figure, self.figure_frame)
        canvas.draw()
        canvas.get_tk_widget().pack(side=Tk.TOP, fill=Tk.BOTH, expand=True)

        # Navigation toolbar above plot
        ...
        self.editor_frame = Frame(self.master, relief=Tk.RAISED)
        self.editor_frame.pack(side=Tk.BOTTOM, fill=Tk.BOTH, expand=True)

        lbl1 = Tk.Label(self.editor_frame, text="Scaling Factor")
        lbl1.pack(padx=5, pady=5)
        self.entry = Tk.Entry(self.editor_frame)
        self.entry.insert(0, '1.0')     # (index, string)
        self.entry.pack(padx=5)
        SendButton = Tk.Button(self.editor_frame, text='Send', command=self.sendFactorToMCU)
